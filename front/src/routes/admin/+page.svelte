<script lang="ts">
 ...
 const exportItems = (filename, which) => {
		var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(Object.fromEntries(which), null, 2));
...
<div>
	<Accordion hover="hover:bg-warning-hover-token">
		<!-- Models -->
		<AccordionItem class="bg-surface-200 rounded my-2" >
			<svelte:fragment slot="lead"><i class="fa icon-orange fa-cube w-6" /></svelte:fragment>
			<svelte:fragment slot="summary">
				<span class="h2 icon-orange">Models</span> ({models.size})
			</svelte:fragment>
			<svelte:fragment slot="content">
				<div>
					<a href="{'#'}" class="variant-filled-error btn btn-sm" type="button" on:click={() => truncateModels()}>
						<span>Truncate Models</span>
						<i class="fa-regular fa-circle-xmark float-right" />
					</a>
					<a href="{'#'}" class="variant-filled-secondary btn btn-sm" type="button" on:click={() => exportItems("models", models)}>
				...
				<Accordion hover="hover:bg-warning-hover-token">
					{#each [...models] as [key, model], i}
						<AccordionItem class="bg-zinc-500 rounded my-2">
							...
							<svelte:fragment slot="content">
								<pre class="rounded p-4 border-zinc-800 bg-zinc-800"><code
										contenteditable="true"
										on:input={()=>{needsSave(i)}}
										on:blur={(event) => {models[key] = event.target.innerText}}>{@html
									Prism.highlight(JSON.stringify(model, null, 2),
		...
		<AccordionItem class="bg-surface-200 rounded my-2" >
			<svelte:fragment slot="lead"><i class="fa icon-orange fa-cube w-6" /></svelte:fragment>
			<svelte:fragment slot="summary"><span class="h2 icon-orange">Printers</span> ({printers.size})</svelte:fragment>
			<svelte:fragment slot="content">
				<div>
					<a href="{'#'}" class="variant-filled-error btn btn-sm" type="button" on:click={() => truncatePrinters()}>
						<span>Truncate Printers</span>
						<i class="fa-regular fa-circle-xmark float-right" />
					</a>
					<a href="{'#'}" class="variant-filled-secondary btn btn-sm" type="button" on:click={() => exportItems("printers", printers)}>
						<span>Export Models</span>
						<i class="fa-regular fa-download float-right" />
					</a>
				</div>
				<Accordion hover="hover:bg-warning-hover-token">
					{#each [...printers] as [key, printer], i}
						<AccordionItem class="bg-zinc-500 rounded my-2">
							<svelte:fragment slot="lead"><i class="fa fa-code text-xl w-6 text-center" /></svelte:fragment>
							<svelte:fragment slot="summary">
								{printer.printerName}
								<div class="float-right">
									<button
							...
							<svelte:fragment slot="content">
								<pre class="rounded p-4 border-zinc-800 bg-zinc-800"><code
									contenteditable="true"
									on:input={()=>{needsSave(i)}}
									on:blur={(event) => {printers[key] = event.target.innerText}}>{@html
									Prism.highlight(JSON.stringify(printer, null, 2),
										Prism.languages.json,
										'json')}</code></pre>
							</svelte:fragment>
						</AccordionItem>
