<script lang="ts">
	...
	let printerStatusInterval = setInterval(function () {
		(async () => {
			let status: { online: string; printerStatus: PrinterStatus; err } =
			...
			if (status.err) {
				...
				errorMessage =
					'Oops!  There was an error connecting to the printer. Will try to reconnect on page refresh.';
				errorVisible = true;
...
<div class="container m-auto w-1/2">
	<div class="flex w-full">
		...
		<div class="w-1/2">
			<button
				type="button"
				class="variant-filled-error btn float-right ml-10"
				disabled={!printerAvailable}
				on:click={confirmDeletePrinter}
			>
	...
	<div class="grid grid-flow-col gap-8">
		<div class="">
			<img src="/mk3s.svg" alt="Printer mk3s.svg" />
		</div>
		<div class="flex flex-col">
			<div>
				<span class="h6 float-right text-xs">*click to edit</span>
			</div>
			<div class="">
				<span class="h4 mr-2">URL:</span>
				<span
					contenteditable="true"
					bind:textContent={printer.url}
					on:input={needsSave}
					class="editable p-1 pr-10"
				>
					{printer.url}
				</span>
			</div>
			<div class="">
				...
				<span
					contenteditable="true"
					bind:textContent={printer.apiKey}
					on:input={needsSave}
					class="editable p-1 pr-10"
				>
					{printer.apiKey}
			...
			<div class="">
				...
				<span
					contenteditable="true"
					bind:textContent={printer.location.name}
					on:input={needsSave}
					class="editable p-1 pr-10"
				>
			...
			<div class="">
				<button
					...
					type="button"
					class="variant-filled-error btn float-right"
					on:click={updatePrinter}
				>
			...
			<div class="">
				{#if online}
					<div class="border-l-8 border-lime-600 bg-neutral-700">
						<div class="ml-4 py-2 text-center text-lime-600">ONLINE</div>
					</div>
				{:else}
					<div class="border-l-8 border-red-800 bg-neutral-700">
						<div class="ml-4 py-2 text-center text-red-700">OFFLINE</div>
					</div>
				{/if}
			</div>
			<hr class="my-4 !border-t-2" />
			<div class="border-l-8 border-yellow-600 bg-neutral-700">
				<div class=" ml-4 py-2 text-center text-neutral-100">
			...
			<div class="grid grid-cols-3">
				<div class="m-auto">
					...
					<RadialGauge
						name="extruder"
						bind:value={printerStatus.temperature.tool0.actual}
						unitSymbol="&#176;"
						min={0}
						max={300}
					></RadialGauge>
				...
				<div class="m-auto">
					...
					<RadialGauge
						name="bed"
						bind:value={printerStatus.temperature.bed.actual}
						unitSymbol="&#176;"
						min={0}
						max={100}
					></RadialGauge>
				...
				<div class="m-auto">
					...
					<RadialGauge
						name="ambient"
						bind:value={printerStatus.temperature.A.actual}
						unitSymbol="&#176;"
						min={0}
						max={100}
					></RadialGauge>
	...
	{#if activeJob}
		...
		<div class="variant-ghost-secondary mb-2 flex w-full rounded p-2">
			<div class="h4 ml-2 w-1/2 font-bold">Job Information</div>
			<div class="w-1/2">
				<button
					type="button"
					class="variant-filled-error btn float-right"
		...
		{#if cancelling}
			<div class=" variant-ghost-warning mb-3 py-3 text-center">
				<span class="text-xl font-bold text-red-800">Cancelling Print Job</span>
			</div>
		{/if}
		<div class="mx-auto grid w-fit grid-cols-2">
			<div class="px-4">
				<div>
					<span class="h5 font-bold">Job: </span>{jobInfo.job.file.name}
				</div>
				<div>
					<span class="font-bold">Estimated Time:</span>
					{SecondsPrettyPrint(jobInfo.job.estimatedPrintTime)}
				</div>
			</div>
			<div class="m-auto">
				...
				<RadialGauge
					name="jobCompletion"
					bind:value={jobInfo.progress.completion}
					unitSymbol="%"
					min={0}
					max={100}
				></RadialGauge>
	...
	<div class="m-auto w-2/3">
		...
		{:then data}
			{#if data.printerFiles.children.length > 0}
				{#each data.printerFiles.children as file}
					<div class="flex flex-row">
									...
									<div class="basis-1/3">
										...
										<div>
											W:{file.gcodeAnalysis.dimensions.width} x L:{file.gcodeAnalysis.dimensions
												.height} x D:{file.gcodeAnalysis.dimensions.depth}
										</div>
									</div>
									<div class="basis-1/6">
										<i class="icon fa-regular fa-clock" />
										<div>
											{new Date(file.gcodeAnalysis.estimatedPrintTime * 1000)
												.toISOString()
												.slice(11, 19)}
										</div>
									</div>
						...
						<div class="basis-1/6">
							<button
								type="button"
								class="variant-filled-error btn float-right ml-10"
								disabled={!printerAvailable || activeJob}
								on:click={() => {
			...
			{:else}
				<h5 class="variant-ghost-warning h6 my-6 rounded border py-1 text-center">
					-- There are no Print Files on this printer --
				</h5>
			{/if}
		{/await}
	</div>
