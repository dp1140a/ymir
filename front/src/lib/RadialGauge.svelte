<script lang="ts">
	...
	function myScale(value: number) {
		...
		let valueAngle = d3.scaleLinear().domain([min, max]).range([startAngle, endAngle]).clamp(true)(
			value
		);
		if (debug) {
			console.log(name);
...
<svg {width} {height} xmlns="http://www.w3.org/2000/svg">
	<path
		d={trackArc()}
		transform="translate({width / 2}, {height / 1.75})"
		class="track"
		bind:this={trackArcEl}
	/>
	<path d={arc()} transform="translate({width / 2}, {height / 1.75})" style="fill: {color}" />
	<text class="range" x={width * 0.15} y={height * 0.92}>{min}</text>
	<text class="range" x={width * 0.8} y={height * 0.92}>{max}</text>
...
{#if debug}
	<div>
		...
		<input type="range" {min} {max} bind:value on:input={setColor} />
		<input type="number" {value} on:change={setColor} />
	</div>
{/if}
